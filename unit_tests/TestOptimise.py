...
class TestOptimise(unittest.TestCase):
    ...
    def test_traj_global_res(self):
        res_func_s1, _ = opt.init_opt_funcs(self.sys1, 2, np.zeros(2))
        res_func_s2, _ = opt.init_opt_funcs(self.sys2, 2, np.zeros(2))
        ...
        # correct value
        gr_t1s1_true = res_funcs.global_residual(self.traj1, self.sys1, self.freq1, np.zeros(2), with_zero = True)
        gr_t2s1_true = res_funcs.global_residual(self.traj2, self.sys1, self.freq2, np.zeros(2), with_zero = True)
        gr_t1s2_true = res_funcs.global_residual(self.traj1, self.sys2, self.freq1, np.zeros(2), with_zero = True)
        gr_t2s2_true = res_funcs.global_residual(self.traj2, self.sys2, self.freq2, np.zeros(2), with_zero = True)
        self.assertAlmostEqual(gr_t1s1, gr_t1s1_true)
        self.assertAlmostEqual(gr_t2s1, gr_t2s1_true)
        self.assertAlmostEqual(gr_t1s2, gr_t1s2_true)
        self.assertAlmostEqual(gr_t2s2, gr_t2s2_true)

    def test_traj_global_res_jac(self):
        _, res_grad_func_s1 = opt.init_opt_funcs(self.sys1, 2, np.zeros(2))
        _, res_grad_func_s2 = opt.init_opt_funcs(self.sys2, 2, np.zeros(2))
        ...
        # correct values
        gr_traj_t1s1_true, gr_freq_t1s1_true = res_funcs.global_residual_grad(self.traj1, self.sys1, self.freq1, np.zeros(2))
        gr_traj_t2s1_true, gr_freq_t2s1_true = res_funcs.global_residual_grad(self.traj2, self.sys1, self.freq2, np.zeros(2))
        gr_traj_t1s2_true, gr_freq_t1s2_true = res_funcs.global_residual_grad(self.traj1, self.sys2, self.freq1, np.zeros(2))
        gr_traj_t2s2_true, gr_freq_t2s2_true = res_funcs.global_residual_grad(self.traj2, self.sys2, self.freq2, np.zeros(2))
