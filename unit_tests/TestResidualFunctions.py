...
class TestResidualFunctions(unittest.TestCase):
    ...
    def gen_gr_grad_FD(traj, sys, freq, mean, step = 1e-6):
        ...
        traj_time = traj_funcs.swap_tf(traj)

        # initialise arrays
        gr_grad_FD_traj = np.zeros(traj_time.shape)

        # loop over trajectory DoFs and use CD scheme
        for i in range(traj_time.shape[0]):
            for j in range(traj_time.shape[1]):
                traj_for = traj_time
                traj_for[i, j] = traj_time[i, j] + step
                traj_for = Trajectory(traj_funcs.swap_tf(traj_for))
                gr_traj_for = res_funcs.global_residual(traj_for, sys, freq, mean)
                traj_back = traj_time
                traj_back[i, j] = traj_time[i, j] - step
                traj_back = Trajectory(traj_funcs.swap_tf(traj_back))
                gr_traj_back = res_funcs.global_residual(traj_back, sys, freq, mean)
                gr_grad_FD_traj[i, j] = (gr_traj_for - gr_traj_back)/(2*step)
        gr_grad_FD_traj = Trajectory(traj_funcs.swap_tf(gr_grad_FD_traj))
